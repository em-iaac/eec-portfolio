...
- Analytics (decided 2026-07-07): GoatCounter, cookieless page counting; no cookies, no consent banner, no personal data. Script in index.html + SPA route counting in App.tsx. Localhost is never counted. Two anonymous events since 2026-09-07 (design D of the screen-time pass, her ruling): `read:<path>` once a page has been visible and focused for 30 s of active time, `out:<door>` on the six outbound doors (sensi, cv-pdf, book-pdf, github, linkedin, email). Both live in PageCount in App.tsx, appear as rows in the dashboard's Pages list (type `read:` or `out:` in Filter paths), and /rights 3.2 stays as written.
